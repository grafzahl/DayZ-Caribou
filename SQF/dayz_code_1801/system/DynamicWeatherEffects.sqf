...
if (isServer) then {
    ...
    if (!isDedicated) then {[0, drn_var_DynamicWeather_Snow] spawn fn_snowfall;};
...
[_snowIntervalSnowProbability, _debug] spawn {
    ...
    if (!isDedicated) then {[0.1, _snow] spawn fn_snowfall;};
    sleep 0.1;
    ...
    while {true} do {
        ...
        if (!isDedicated) then {[3, _snow] spawn fn_snowfall;};
