private ["_item","_trees","_findNearestTree","_tree","_objInfo","_lenInfo","_objName","_i","_k","_distance2d","_distance3d","_isForest","_countOut","_counter","_isOk","_proceed","_dis","_sfx","_animState","_started","_finished","_isMedic","_itemtodrop","_nearByPile"];
...
_isForest = ["forest",dayz_surfaceType] call fnc_inString;
if (_isForest) then {
	_distance2d = 4;
	_distance3d = 7;
} else {
...
diag_log format["%1 tree: %2, isforest: %3, _distance2d: %4, _distance3d: %5",__FILE__,_tree,_isForest,_distance2d,_distance3d];
if (_distance2d <= 5) then {
		...
		if (_proceed) then {
			...
			//if ("" == typeOf _tree) then { _tree setDamage 1; }
