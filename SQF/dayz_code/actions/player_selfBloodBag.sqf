...
_bloodAmount = round(2000 + (random 3000)); // Amount of blood to give to player
...
_infectedLifeLostPart = round(1000 + (random 500));
_infectedLifeLost = _bloodAmount + _infectedLifeLostPart; // Amount of life to loose in becomes infected
...
if(_playerHumanity >= 5000) then {
	_bloodAmount = round(4000 + (random 3000));
	_bloodbagUseTime = 20;
	_infectionChance = 5;
	_bloodbagLastUsedTime = 120;
};
...
if (_inCombat) then { // Check if in combat
	...
	if (_finished) then {
		...
		if (random(_infectionChance) < 1) then {
			...
			cutText [format["\n\nBloodbag was infected! You lost %1ml blood!",_infectedLifeLostPart], "PLAIN DOWN"]; //display text at bottom center of screen if infected
			...
			cutText [format["\n\nBloodbag was fresh and clean! You gained %1ml blood!",_bloodAmount], "PLAIN DOWN"]; //display text at bottom center of screen on succesful self bloodbag
		};
		...
		cutText [format["\n\nYou have interrupted giving yourself a bloodbag!"], "PLAIN DOWN"]; //display text at bottom center of screen on interrupt
	};
};